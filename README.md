<h1 align="center">Hi 👋, I'm Bhargab Deka</h1>
<h3 align="center">
Full Stack Developer | MERN & Next.js Developer
</h3>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com/?font=Fira+Code&size=26&pause=1000&center=true&vCenter=true&width=700&lines=Full+Stack+Developer;MERN+Stack+Developer;Next.js+Developer;Backend+Developer" />
</p>

---

## 🚀 About Me
- 🎓 Final Year B.Tech CSE Student at Jorhat Engineering College
- 💻 Building and shipping production full-stack web applications, solo and in teams
- ⚡ Focused on MERN Stack, Next.js, and backend systems
- 🛠 Interested in system design and modern web technologies

---

## 🌐 Connect With Me
<p align="left">
  <a href="https://www.linkedin.com/in/bhargab-deka-417126200" target="_blank">
    <img src="https://skillicons.dev/icons?i=linkedin" />
  </a>
  <a href="https://github.com/bhargabdeka-deka" target="_blank">
    <img src="https://skillicons.dev/icons?i=github" />
  </a>
  <a href="mailto:bhargab1234deka@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" />
  </a>
</p>

---

## 💻 Tech Stack
<p align="left">
  <img src="https://skillicons.dev/icons?i=html,css,js,ts,react,nextjs,nodejs,express,mongodb,postgresql,redis,java,python,c,cpp,git,github,tailwind,vscode,docker" />
</p>

---

## 🛠 Core Skills
Full Stack Web Development · REST API Design · JWT Authentication · Role-Based Access Control (RBAC) · Database Design · Backend Development · System Design

---

## 🚀 Featured Projects

### 🛡 [CyberShield](https://cybershield-green-two.vercel.app/)
Cyber-crime intelligence & reporting platform (MERN Stack) with a self-designed 0–100 risk-scoring algorithm, JWT auth, RBAC, and a spam-deterring trust system.
**[Live Demo](https://cybershield-green-two.vercel.app/)**

### 🏨 [ORION Hostel Portal](https://www.orionjech7.site/)
Institutional management platform (Next.js, TypeScript, Supabase, PostgreSQL) — live and in active use by 75+ residents and 800+ alumni.
**[Live Site](https://www.orionjech7.site/)**

### 🎤 InterviewOS
Real-time technical interview platform. Owned the Next.js frontend and Electron.js desktop integration; led testing/QA. Backend by teammates (Go, WebRTC, Redis, Docker).
**[GitHub Repo](https://github.com/bhargabdeka-deka/interview_help)**

### ⚡ Smart Electricity Management System
MERN-based platform (APDCL internship) for KYC verification, complaint handling, and role-based administration.
**[GitHub Repo](https://github.com/bhargabdeka-deka/smart-electricity-management-system)**
---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=bhargabdeka-deka&label=Profile%20Views&color=0e75b6&style=flat" />
</p>
